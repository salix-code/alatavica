    ...
    def run(self):
        ...
        while index < len(rows):
            if index == 0:
                ...
                low_price =  get_low_price(min(rows[max(0,index - 4):index + 1],key=lambda x: x.low_price))
                high_price = get_high_price(max(rows[max(0,index - 4):index + 1], key=lambda x: x.high_price))
        ...
        while index < len(rows):
            if data[index] < rows[index].low_price:
                select_index = index
                while index < len(rows) and data[index] < rows[index].low_price:
                    index += 1
